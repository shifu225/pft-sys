...
async def require_evaluator(
    ...
    Admins and Super Admins should NOT access evaluator endpoints.
    """
    if current_user.role != "evaluator":  # Strict check - only evaluator
        raise HTTPException(
            status_code=403,
            detail="Evaluator access required. Admins cannot perform evaluations."
        )
    return current_user
