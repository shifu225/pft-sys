...
# GET ALL RESULTS - ADMIN ONLY (Evaluators should not see all records)
@router.get("/pft-results", response_model=List[dict])
async def get_all_pft_results(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin)  # Changed to require_admin
):
...
async def get_pft_result_by_id(
    result_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin)  # Restricted to admin
):
...
# GET RESULTS BY SERVICE NUMBER - ADMIN ONLY (for security)
